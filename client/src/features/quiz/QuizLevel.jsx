...
export default function QuizLevel() {
    ...
    // Get question count from URL params, default to 10 if not provided
                        ...
                        <div className="relative bg-card/90 backdrop-blur-2xl border border-border/50 rounded-3xl p-8 shadow-2xl">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-8">
                                <div className={`p-3 rounded-xl ${colorClasses[selectedLevel.color].bgIcon} border ${colorClasses[selectedLevel.color].border} text-center`}>
                                    ...
                                    <span className={`text-xl font-black ${colorClasses[selectedLevel.color].icon}`}>{questionCount}</span>
                                </div>
                                <div className={`p-3 rounded-xl ${colorClasses[selectedLevel.color].bgIcon} border ${colorClasses[selectedLevel.color].border} text-center`}>
                                    <span className="block text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">Duration</span>
                                    <span className={`text-xl font-black ${colorClasses[selectedLevel.color].icon}`}>{Math.ceil(questionCount * 1.5)}m</span>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    onClick={() => {
                                        // Clear any existing quiz data for this topic/level
                                        const storageKey = `quiz_${topic}_${selectedLevel.name.toLowerCase()}`;
                                        ...
                                        localStorage.removeItem(`${storageKey}_result`);
                                        
                                        // Navigate directly to quiz with auto-start and question count
                                        navigate(`/quiz/${topic}/${selectedLevel.name.toLowerCase()}?autostart=true&count=${questionCount}`);
                                    }}
